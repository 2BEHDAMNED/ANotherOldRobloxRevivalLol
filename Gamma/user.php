<?php
	...
	if(isset($_POST['__EVENTTARGET'])) {
		if(!$private_profile && $user != null && $_POST['__EVENTTARGET'] == 'ctl00$cphRoblox$rbxUser$acceptFriend$Submit') {
			$user->Friend($get_user);
		} else if($user != null && $_POST['__EVENTTARGET'] == 'ctl00$cphRoblox$rbxUser$removeFriend$Submit') {
			$user->Unfriend($get_user);
		}
	}
	...
	$status_string = $get_user->IsOnline() ? "Online" : "Offline";
	$status_label = $get_user->GetOnlineActivity();

	$user_friends = $get_user->GetFriends();
	$user_friends_count = count($user_friends);
	$all_friends_count = $user_friends_count;

	$visit_count = 0;

	if($user != null) {
		$friends_with = $user->IsFriendsWith($get_user);
	}
?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" id="gamma-lambda-cam">
	<head>
		...
		<link id="ctl00_Imports" rel="stylesheet" type="text/css" href="/CSS/GammaCSS.css">
		<link id="ctl00_Favicon" rel="Shortcut Icon" type="image/ico" href="/gammafavicon.ico">
		...
		<script src="/js/Gamma/Stuff.js?t=<?= time() ?>"></script>
		<script src="/js/Gamma/PlaceLauncher.js?t=<?= time() ?>"></script>
		<style>
			.ProfileLink {
				padding : 0 10px;
			}
												...
												<p>
													...
													<span id="ctl00_cphRoblox_rbxUserPane_rbxPublicUser_lBlurb">
														<table style="width:241px">
															<tbody>
																<tr class="ProfileLink"><td><a href="/Gamma/My/Inbox.aspx">Inbox</a></td></tr>
																<tr class="ProfileLink"><td><a href="/Gamma/My/Character.aspx">Change Character</a></td></tr>
																<tr class="ProfileLink"><td><a href="/Gamma/My/Profile.aspx">Edit Profile</a></td></tr>
																<tr class="ProfileLink"><td><a href="/Gamma/User.aspx?ID=<?= $get_user->id ?>">View Public Profile</a></td></tr>
								...
								<div id="UserBadges">
									...
									<table id="ctl00_cphRoblox_rbxUserBadgesPane_dlBadges" cellspacing="0" align="Center" border="0">
										<tbody>
											<?php 
											$badge_count = 0;
												if(count($get_user->GetProfileBadges()) != 0) {
													foreach($get_user->GetProfileBadges() as $badge) {
														if(!($badge instanceof ANORRLBadge)) {
															continue;
														}
														if($badge_count == 0) {
															echo <<<EOT
							...
							<div id="UserStatisticsPane">
								<div id="UserStatistics">
									...
									<div class="Statistic">
										...
										<div class="Value"><span id="ctl00_cphRoblox_rbxUserStatisticsPane_lFriendsStatistics"><?= $all_friends_count ?> </span></div>
									</div>
									<div class="Statistic">
									...
									<div class="Statistic">
										...
										<div class="Value"><span id="ctl00_cphRoblox_rbxUserStatisticsPane_lKillsStatistics"><?= 0//$get_user->GetKillCount() ?> (nil last week)</span></div>
									</div>
								</div>
							</div>
						</div>
											...
											foreach($places as $place) {
												...
												<div class="place-container">
													...
													<div class="Place">
														...
														<div class="Thumbnail">
															<a id="ctl00_cphRoblox_rbxUserPlacesPane_ctl02_rbxPlatform_rbxPlaceThumbnail" disabled="disabled" title="$place_name" href="/Item.aspx?ID=$place_id" style="display:inline-block;"><img src="/thumbs/?id=$place_id&sx=420&sy=230" border="0" alt="$place_name" blankurl="http://t1.roblox.com:80/blank-420x230.gif"></a>
														</div>
