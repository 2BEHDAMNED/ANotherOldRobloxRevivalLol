...
function LoadAssetMenu(userid, menuName, page) {
	...
	$.get( "/api/stuff?c="+menuName+"&uid="+userid+"&page="+page, function( data ) {
		...
		$("#ctl00_cphRoblox_rbxUserAssetsPane_CatalogHyperLink").attr("href", "/Gamma/Catalog.aspx?c="+menuName);
		if(Object.keys(data["assets"]).length != 0) {
			...
			if(Object.keys(data["assets"]).length < 5) {
...
function unfavouriteAsset(asset_id) {
    $.post( "/api/favourite", { asset: asset_id }).done(function() {
        $("#asset_"+asset_id).remove();
		LoadFavourites(anorrl_userid, anorrl_favtype, anorrl_page);
    });
}

var anorrl_userid = -1;
var anorrl_favtype = 0;
var anorrl_page = 1;

function LoadFavourites(userid, type, page) {
	anorrl_userid = userid;
	if(page === undefined) {
		page = anorrl_page;
	}
	anorrl_page = page;
	...
	$.get( "/api/favourites?getfavlist&id="+userid+"&type="+type+"&page="+page, function( data ) {
		...
		if(Object.keys(data).length != 0) {
			...
			for (var key in data){
				...
				creatornamelink.attr("href","/Gamma/User.aspx?ID="+asset["CreatorUserID"]);
