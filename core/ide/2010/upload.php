<?php
	...
	if($user == null) {
		die("Hey you need to login by the way!!");
	}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body bgcolor="buttonface" scroll="no">
		<form name="PublishContent" method="post" action="/IDE/2010/Upload.aspx" id="PublishContent">
			<input id="DialogResult" type="hidden" />		    
			<table height="100%" cellpadding="12" width="100%">
				<tr valign="top">
					<td colspan="2">
					  <p>You are about to publish this Place to ANORRL.  Please choose how you would like to save your work:</p>
					</td>
				</tr>
				<tr>
					...
					<td valign="top"><strong>Create a new Place on ANORRL.</strong><br />Choose this to create a new Place!</td>
				</tr>
				<tr>
					...
					<td valign="top"><strong>Update an existing Place on ANORRL.</strong><br />Choose this to make changes to a Place you have previously created.  You will have the opportunity to select which Place you wish to update.</td>
				</tr>
				<tr>
	...
	<body bgcolor="buttonface" scroll="no">
		<form name="PublishContent" method="post" action="Upload.aspx" id="PublishContent">
			...
			<script type="text/javascript">
				function uploadData() {
					try {
						window.external.Write().Upload("http://<?= $domain ?>/Data/Upload.ashx?assetid=0&type=Place&name=<?= $name ?>&description=<?= $desc ?>&ispublic=<?= $ispublic ?>&iscopylocked=<?= $iscopylocked ?>&year=2010");
	...
	<body bgcolor="buttonface" scroll="no">
		<form name="PublishContent" method="post" action="Upload.aspx" id="PublishContent">
			...
			<script type="text/javascript">
				function uploadData() {
					try {
						window.external.Write().Upload("http://<?= $domain ?>/Data/Upload.ashx?assetid=<?= $placeid ?>&type=Place&name=&description=&ispublic=False&year=2010");
						document.getElementById("Uploading").style.display='none';
						document.getElementById("Confirmation").style.display='block';
					} catch (ex) {
						window.alert(ex.message);
