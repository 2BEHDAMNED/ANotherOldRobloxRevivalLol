	...
	class NewAssetUploader {
		...
		public static function UploadAsset(
			...
			if($user != null && !$user->IsBanned()) {
				...
				if(is_array($file)) {
					if($file['error'] != 0) {
						return ["error" => true, "reason" => "Something went wrong! Server could be full?"];
					}

					if($file['size'] > 62914560) {
						return ["error" => true, "reason" => "File was too large! Only 60MB is allowed!"];
					}
					...
					if($data != null) {
						if(AssetTypeUtils::IsRBX($type)) {
							...
							if(!self::IsValidXML($data, $legacy)) {
								return ["error" => true, "reason" => "File format not valid!"];
							}
								...
								if(!$result['error']) {
									...
									if(AssetTypeUtils::IsRenderable($type)) {
										...
										if(!file_exists($assetsdir)) {
											$render = self::GetRender($result['id'], $type);
											if($render != null) {
